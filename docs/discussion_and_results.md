...
Our results for the Bundesliga (+4.46% ROI) compare favorably with existing literature. Start-of-the-art supervised learning approaches (XGBoost, Random Forest) often achieve accuracies around 45-50% but struggle to maintain positive ROI due to bookmaker margins (typically 5-8%) [1], [5]. In contrast, our RL-based approach directly optimizes for financial reward rather than accuracy. While Rahimian et al. [7] reported success using offline RL, our online DQN approach demonstrates that active exploration in a simulated betting environment can discover profitable inefficiencies, particularly in high-scoring leagues like the Bundesliga.

## F. Analysis of the Best Model (Bundesliga)
